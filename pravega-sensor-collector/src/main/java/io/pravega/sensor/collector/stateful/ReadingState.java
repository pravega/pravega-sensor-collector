...
public class ReadingState {
    ...
    public ReadingState(Connection connection){
        ...
        try {
            try (final Statement statement = connection.createStatement()) {
                statement.execute(
                        "create table if not exists LastReadingState (" +
    ...
    public void updateState(String state) {
        try (final PreparedStatement updateStateStatement = 
            connection.prepareStatement("update LastReadingState set lastTimestamp = ?"))
        {
